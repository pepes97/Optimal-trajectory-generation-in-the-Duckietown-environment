...
class FrenetTrajectoryPlanner:
    def __init__(self, p0: (float, float, float), s0: (float, float, float), s_target: Frenet = None):
        ...
        self.opt_path_tot = min(self.paths, key=attrgetter('ctot')); # store the best path for ctot
        self.opt_path_ct = min(self.paths, key=attrgetter('ct')); # store the best path for ct
