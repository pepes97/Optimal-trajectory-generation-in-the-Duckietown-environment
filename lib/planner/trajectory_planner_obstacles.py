...
tpdp = TrajectoryPlannerDefaultParamsDTObstacles

class TrajectoryPlannerParamsDTObstacles:
...
class TrajectoryPlannerV1DTObstacles(Planner):
    def __init__(self, *args, **kwargs):
        # Load parameters if TrajectoryPlannerParams object is passed
        if args is not None and len(args) > 0:
            if isinstance(args[0], TrajectoryPlannerParamsDTObstacles):
