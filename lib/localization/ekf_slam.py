...
class EKF_SLAM():
    ...
    def step(self, inputs: np.ndarray = np.zeros((CONTROL_DIM,),dtype=np.float32), \
        ...
        self.predict(inputs = inputs)
        # incorporate new measurement
        self.correct(observed = observed)

    def predict(self, inputs: np.ndarray = np.zeros((CONTROL_DIM,),dtype=np.float32)):
    ...
    def input_noise(self, inputs: np.ndarray = np.zeros((CONTROL_DIM,),dtype=np.float32), \
        ...
        # Inject noise into model inputs
        # control noise has a constant part which is sigma_u and some
        # velocity dependent parts
        sigma_v = inputs[0]; # translational velocity dependent part
        sigma_w = inputs[1]; # rotational velocity dependent part
    ...
    def measure_noise(self, C: np.ndarray = np.identity(OBSERV_DIM,dtype=np.float32)):
    ...
    def cost_matrix(self, landmarks: np.ndarray = np.zeros((1,LANDMARK_DIM),dtype=np.float32), \
                    observed: np.ndarray = np.zeros((1,OBSERV_DIM),dtype=np.float32)):
        # cost matrix is A is (M x N)
        # get number of landmarks and new observations
        m_dim = observed.shape[0]
        ...
        for n in range(n_dim):
            ...
            omega_mn = self.measure_noise(C = C)
            #for all measurements
            for m in range(m_dim):
                # obtain current measurement
                z = observed[m,:].reshape((OBSERV_DIM,1))
    ...
    def best_friend(self, associations: np.ndarray = np.zeros((1,ASSOCIATION_DIM),dtype=np.float32), \
        ...
        # Best friends: an
        # association should be the
        # best (i.e. minimum) of
        # both row and column
        # landmark in the state
        if associations.shape[0] == 0 : np.array([]).reshape(0,ASSOCIATION_DIM), np.array([])
        # min by columns
    ...
    def prune_heuristics(self, A: np.ndarray = np.identity(2,dtype=np.float32)):
        # Bad associations are EVIL
        # To avoid the above cases
    ...
    def associate(self, landmarks: np.ndarray = np.zeros((1,LANDMARK_DIM),dtype=np.float32), \
        ...
        A = self.cost_matrix(landmarks = landmarks, observed = observed)
    ...
    def add_landmarks(self, new_indices: np.ndarray = np.zeros((1,),dtype=np.int32), \
        ...
        new_landmarks = observed[new_indices,:]
        ...
        landmark_index = self.mu.shape[0]
        ...
        self.sigma[landmark_index:landmark_index+LANDMARK_DIM * num_new, landmark_index:landmark_index+LANDMARK_DIM * num_new] = sigma_landmark